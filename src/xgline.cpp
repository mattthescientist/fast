...
string XgLine::getLineString () {
  ...
  oss.width (9);  oss.precision (4); oss << fixed << right << width ();
  ...
  oss << left << " " << IdLong;
  oss.width (11); oss.precision (6); oss << fixed << right << airWavelength ();
  return oss.str();
}
...
void XgLine::noise (double NewNoise) throw (Error) {
  if (NewNoise < 0.0) { throw Error (LINE_NEGATIVE_NOISE); }
  Noise = NewNoise;
}
